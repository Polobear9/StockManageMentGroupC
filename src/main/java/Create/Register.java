...
public class Register {
	...
	private static void registrationDate(Scanner sc) {
		while(true) {
			...
			if (isRight("(\\d{4})(1[0-2]|0[1-9])(0[1-9]|[12][0-9]|3[01])" , registrationDate) == false) {
	...
	private static void purchasePrice(Scanner sc) {
		while(true) {
			...
			if (isRight("\\d{0,}", purchasePrice) == false) {
				System.out.println("販売単価は半角数字で入力してください。");
				continue;
			}
			if (isRight("[\\d]{0,8}", purchasePrice) == false) {
	...
	private static void sellingPrice(Scanner sc) {
		while (true) {
			...
			if (isRight("\\d{0,}", sellingPrice) == false) {
				System.out.println("販売単価は半角数字で入力してください。");
				continue;
			}
			if (isRight("[\\d]{0,8}", sellingPrice) == false) {
				System.out.println("販売単価は８桁以下で入力してください。");
				continue;
			}
			break;
	...
	private static void produCategory(Scanner sc) {
		while(true) {
			...
			if (isRight(".+", produCategory) == false) {
	...
	private static void producCode(Scanner sc) {
		while(true) {
			...
			if(isRight("\\d{0,}", productCode) == false) {
	...
	private static void productId(Scanner sc) {
		while(true) {
			...
			if (isRight("[\\w-]{0,}", productId) == false) {
	...
	private static boolean isRight(String regular , String target) {
		Pattern pattern = Pattern.compile(regular); //\\w{10}
